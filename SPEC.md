...
- **ローカル (リアルタイム)**: D-FINE nano (ONNX) を on-device で実行、ライブカメラに bbox を毎フレーム (~3fps) 重畳 → タップでカゴ追加。COCO 80 クラス、Gemini 不要、ネットワーク不要。iOS Safari ではメモリ上限により ~15s で落ちる既知の制約あり。
