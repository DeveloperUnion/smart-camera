...
const TALK_SYSTEM_INSTRUCTION = [
  ...
  '',
  '【物体の特定】「これ」「それ」「このモニター」のように指された物は、今カメラに大きく写っている物体だと解釈し、具体的な名称（例: モニター、液晶ディスプレイ、電子レンジ、ソファ、段ボール）を add_to_cart の name に入れる。ユーザーが名前を言わなくても、映像から判断して特定してよい。物体の画面上の位置や座標を答える必要はない（切り抜きは後段の処理が担当する）。あなたは名称と個数の特定に専念する。',
  '',
